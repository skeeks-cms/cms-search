    ...
    <div class="col-sm-8 news-v3">
        <div class="news-v3-in-sm no-padding">
            ...
            <ul class="list-inline posted-info">
                <? if ($model->createdBy) : ?>
                    <li><?= \Yii::t('skeeks/search', 'Added') ?>: <a href="<?= $model->createdBy->getPageUrl(); ?>"
                                                                     title="<?= $model->createdBy->name; ?>"><?= $model->createdBy->name; ?></a>
                    </li>
                <? endif; ?>
                <? if ($model->cmsTree) : ?>
                    <li><?= \Yii::t('skeeks/search', 'Category') ?>: <a href="<?= $model->cmsTree->url; ?>"
                                                                        title="<?= $model->cmsTree->name; ?>"><?= $model->cmsTree->name; ?></a>
                    </li>
                <? endif; ?>
                <li><?= \Yii::t('skeeks/search', 'Time of publication') ?>
                    : <?= \Yii::$app->formatter->asDate($model->published_at, 'full') ?></li>
